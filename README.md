...
遇到System32，仍然是加入%SystemRoot%\\变量即可，也就是较为合理的处理逻辑。
### （4）获取对应路径的详细信息
...
可以看到，是存在一些路径的，直接处理掉即可。

也有可能这里也没有，那么就是找不到目标程序，需要加一个tag即可。或者直接跳过处理。
